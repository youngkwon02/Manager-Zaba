<?php
    class calendarDAO {
        ...
        function get_myDate($user_email, $now_Y, $now_M) {
            $db_connect = mysqli_connect(self::$db_host, self::$db_user, self::$db_passwd, self::$db_name);

            $search_start = $now_Y.'-'.($now_M-1).'-01';
            $search_end = $now_Y.'-'.($now_M+1).'-31';
            $query = "SELECT CAL_seq, title, owner, start_date, end_date, color FROM CALENDAR WHERE owner = '".$user_email."' AND ( '".$search_end."' > start_date OR '".$search_start."' < end_date ) ORDER BY start_date and end_date and record_date";
        ...
        function get_allDate($user_email, $allFriendsList, $now_Y, $now_M) {
            $db_connect = mysqli_connect(self::$db_host, self::$db_user, self::$db_passwd, self::$db_name);
